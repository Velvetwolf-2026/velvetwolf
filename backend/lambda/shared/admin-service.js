...
export async function getAdminDashboardStats() {
  ...
  if (ordersResult.error) {
    logError("Dashboard orders query failed", adminLogContext({ error: ordersResult.error }));
    throw new ApiError(500, "Failed to load dashboard data.");
  }
  if (customersResult.error) {
    logError("Dashboard customers query failed", adminLogContext({ error: customersResult.error }));
    throw new ApiError(500, "Failed to load dashboard data.");
  }
  if (productsResult.error) {
    logError("Dashboard products query failed", adminLogContext({ error: productsResult.error }));
    throw new ApiError(500, "Failed to load dashboard data.");
  }
...
export async function getAdminOrders({ status, page = 1, limit = 50 } = {}) {
  ...
  if (error) {
    logError("Admin orders query failed", adminLogContext({ status, page, limit, error }));
    throw new ApiError(500, "Failed to load orders.");
  }

  // Enrich with customer name from users table via user_id
...
export async function updateAdminOrderStatus(orderId, status, adminId) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .update(updates)
    .eq("id", orderId)
    .select("id, status, dispatched_at, delivered_at")
    .single();

  if (error) {
    logError("Order status update failed", adminLogContext({ orderId, status, adminId, error }));
    throw new ApiError(500, "Failed to update order status.");
  }

  if (!data) {
    logWarn("Order not found for status update", adminLogContext({ orderId, status }));
...
export async function getAdminProducts({ collection, search, page = 1, limit = 100 } = {}) {
  ...
  if (error) {
    logError("Admin products query failed", adminLogContext({ collection, search, error }));
...
export async function createAdminProduct(productData, adminId) {
  ...
  if (!collection?.trim()) throw new ApiError(400, "Collection is required.");
  if (!Number.isFinite(Number(price)) || Number(price) <= 0) throw new ApiError(400, "Valid price is required.");
  ...
  if (error) {
    logError("Product create failed", adminLogContext({ name, adminId, error }));
...
export async function updateAdminProduct(productId, productData, adminId) {
  ...
  if (error) {
    logError("Product update failed", adminLogContext({ productId, adminId, error }));
...
export async function deleteAdminProduct(productId, adminId) {
  ...
  if (error) {
    logError("Product delete failed", adminLogContext({ productId, adminId, error }));
...
export async function getAdminCustomers({ page = 1, limit = 50, search } = {}) {
  ...
  if (error) {
    logError("Admin customers query failed", adminLogContext({ search, error }));
    throw new ApiError(500, "Failed to load customers.");
  }
...
export async function getAdminAnalytics() {
  ...
  if (recentOrdersResult.error || allOrdersResult.error || topProductsResult.error) {
    logError("Analytics query failed", adminLogContext({
      recentError: recentOrdersResult.error,
      allError: allOrdersResult.error,
      topError: topProductsResult.error,
    }));
    throw new ApiError(500, "Failed to load analytics.");
  }

  // Daily revenue — last 30 days bucketed by day
  const dailyMap = {};
  for (const o of recentOrdersResult.data || []) {
    const day = o.created_at.slice(0, 10); // "YYYY-MM-DD"
    if (!dailyMap[day]) dailyMap[day] = 0;
    dailyMap[day] += Number(o.total_amount || 0);
  }
  const dailyRevenue = Object.entries(dailyMap)
    .map(([date, revenue]) => ({ date, revenue }))
    .sort((a, b) => a.date.localeCompare(b.date));

  // Monthly revenue — last 12 months bucketed by month
  const monthlyMap = {};
  for (const o of allOrdersResult.data || []) {
    const month = o.created_at.slice(0, 7); // "YYYY-MM"
    if (!monthlyMap[month]) monthlyMap[month] = 0;
    monthlyMap[month] += Number(o.total_amount || 0);
  }
