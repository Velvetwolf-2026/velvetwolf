...
export default function TrackOrder() {
  ...
  const handleTrack = () => {
    ...
    if (!ORDER_ID_RE.test(normalizedOrderId)) nextErrors.orderId = "Enter a valid order ID like VW-2025-00123.";
    if (!EMAIL_RE.test(normalizedEmail)) nextErrors.email = "Enter a valid email address.";
      ...
      <div style={S.wrap}>
        ...
        <div style={{ background: surface, border: `1px solid ${border}`, padding: "32px", marginBottom: 48 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
            {[
              { key: "orderId", label: "ORDER ID", placeholder: "VW-2025-00123", val: orderId, set: setOrderId, type: "text" },
              { key: "email", label: "EMAIL ADDRESS", placeholder: "you@email.com", val: email, set: setEmail, type: "email" },
            ].map((f) => (
              <div key={f.key}>
                ...
                <input
                  value={f.val}
                  onChange={(e) => {
                    f.set(f.key === "orderId" ? e.target.value.toUpperCase() : e.target.value);
          ...
          <div style={{ border: `1px solid ${gold}44`, padding: "28px 28px 32px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
              <div>
                ...
                <div style={{ fontFamily: "'Bebas Neue',cursive", fontSize: 22, letterSpacing: 3, color: gold }}>{orderId || "VW-2025-00123"}</div>
              </div>
        ...
        <Sec title="NEED HELP?">
          <p style={S.p}>If your tracking hasn't updated in 48 hours, contact us at <a href="mailto:support@velvetwolf.in" style={{ color: gold }}>support@velvetwolf.in</a> with your order ID. We respond within 4 hours.</p>
        </Sec>
      </div>
    </div>
  );
