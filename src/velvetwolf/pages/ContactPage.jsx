...
const CONTACTS = [
  { icon: "@", label: "GENERAL ENQUIRIES", val: "hello@velvetwolf.in", href: "mailto:hello@velvetwolf.in" },
  { icon: "OD", label: "ORDER SUPPORT", val: "orders@velvetwolf.in", href: "mailto:orders@velvetwolf.in" },
  { icon: "RT", label: "RETURNS & EXCHANGE", val: "returns@velvetwolf.in", href: "mailto:returns@velvetwolf.in" },
  { icon: "BZ", label: "BULK & CORPORATE", val: "bulk@velvetwolf.in", href: "mailto:bulk@velvetwolf.in" },
  { icon: "WA", label: "WHATSAPP SUPPORT", val: "+91 98765 43210", href: "https://wa.me/919876543210" },
];
...
export default function ContactPage() {
  ...
  const validateForm = () => {
    ...
    if (!EMAIL_RE.test(form.email.trim().toLowerCase())) nextErrors.email = "Please enter a valid email address.";
    if (!form.subject) nextErrors.subject = "Please select a subject.";
    if (form.message.trim().length < 10) nextErrors.message = "Please enter a message with at least 10 characters.";
            ...
            {CONTACTS.map((c, i) => (
              <div key={i} style={{ display: "flex", gap: 14, marginBottom: 20, paddingBottom: 20, borderBottom: `1px solid ${border}` }}>
                ...
                <div>
                  ...
                  <a href={c.href} target={c.href.startsWith("http") ? "_blank" : undefined} rel={c.href.startsWith("http") ? "noreferrer" : undefined} style={{ fontSize: 15, color: gold, textDecoration: "none" }}>{c.val}</a>
                </div>
              </div>
            ))}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {SOCIALS.map((s) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    style={{ fontFamily: "'Roboto', sans-serif", fontSize: 10, letterSpacing: 2, padding: "5px 10px", border: `1px solid ${border}`, color: muted, cursor: "pointer", transition: "all 0.2s", textDecoration: "none" }}
                    onMouseEnter={(e) => {
            ...
            {sent ? (
              ...
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                  {[
                    ...
                    <div key={f.key}>
                      ...
                      />
                      {errors[f.key] && <div style={{ marginTop: 8, fontSize: 12, color: "#ff8a80" }}>{errors[f.key]}</div>}
                    </div>
                  ))}
                </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  {errors.subject && <div style={{ marginTop: 8, fontSize: 12, color: "#ff8a80" }}>{errors.subject}</div>}
                </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <textarea
                    ...
                    onBlur={(e) => (e.target.style.borderColor = border)}
                  />
                  {errors.message && <div style={{ marginTop: 8, fontSize: 12, color: "#ff8a80" }}>{errors.message}</div>}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <button
                    ...
                    disabled={sending}
                    style={{ background: gold, color: "#0a0a0a", border: "none", padding: "13px 32px", fontFamily: "'Bebas Neue',cursive", fontSize: 16, letterSpacing: 4, cursor: sending ? "not-allowed" : "pointer", opacity: sending ? 0.7 : 1 }}
                    onMouseEnter={(e) => { if (!sending) e.currentTarget.style.background = goldLight; }}
            ...
            <div key={i} style={{ background: surface, border: `1px solid ${border}`, padding: "18px 16px" }}>
              ...
              {b.lines.map((l, j) => (
                <p key={j} style={{ ...S.p, marginBottom: 3, fontSize: 14 }}>
                  {b.label === "GSTIN" && l.includes("@") ? <a href={`mailto:${l}`} style={{ color: gold }}>{l}</a> : l}
                </p>
              ))}
            </div>
          ))}
