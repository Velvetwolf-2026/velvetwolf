      ...
      <div style={{ background: "var(--graphite)", border: "1px solid var(--smoke)" }}>
        {loading ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {orders.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: "40px 16px", textAlign: "center", fontFamily: "'Roboto', sans-serif", fontSize: 14, fontStyle: "italic", color: "var(--silver)" }}>No orders found</td></tr>
              ) : orders.map((o) => (
                <tr key={o.id} style={{ borderBottom: "1px solid var(--smoke)", opacity: updatingId === o.id ? 0.6 : 1 }}>
                  <td style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--gold)", padding: "14px 16px", letterSpacing: 1 }}>{String(o.id).slice(0, 8).toUpperCase()}</td>
                  <td style={{ padding: "14px 16px" }}>
                    <div style={{ fontFamily: "var(--font-serif)", fontSize: 14, color: "var(--ash)" }}>{o.customerName}</div>
                    <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--silver)", marginTop: 2 }}>{o.customerEmail}</div>
                  </td>
                  <td style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--silver)", padding: "14px 16px" }}>{new Date(o.created_at).toLocaleDateString("en-IN")}</td>
                  <td style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--ash)", padding: "14px 16px" }}>{(o.order_items || []).length}</td>
