      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 60, marginBottom: 60 }}>
          <div>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 8, letterSpacing: 4, color: "var(--gold)", marginBottom: 20 }}>LUXURY STREETWEAR · EST. 2025</div>
            <p style={{ fontFamily: "var(--font-serif)", fontSize: 14, color: "var(--silver)", lineHeight: 1.8, fontStyle: "italic" }}>
            ...
            <div style={{ display: "flex", gap: 14, marginTop: 24 }}>
              {["📸 Instagram", "𝕏 Twitter", "▶ YouTube"].map(s => (
                <span key={s} style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--silver)", cursor: "pointer", letterSpacing: 1 }}>{s}</span>
              ))}
            </div>
          </div>
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: 3, color: "var(--gold)", marginBottom: 20 }}>SHOP</div>
            {[["All Products", "shop"], ["Custom Design", "custom"], ["Bulk Orders", "bulk"], ["Collections", "collection"]].map(([label, pg]) => (
              <div key={label} onClick={() => setPage(pg)} style={{ fontFamily: "var(--font-serif)", fontSize: 14, color: "var(--silver)", cursor: "pointer", marginBottom: 10 }}>{label}</div>
            ))}
          </div>
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: 3, color: "var(--gold)", marginBottom: 20 }}>SUPPORT</div>
            {["Size Guide", "Track Order", "Returns & Exchange", "FAQ", "Contact Us"].map(l => (
              <div key={l} style={{ fontFamily: "var(--font-serif)", fontSize: 14, color: "var(--silver)", cursor: "pointer", marginBottom: 10 }}>{l}</div>
            ))}
          </div>
          <div>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: 3, color: "var(--gold)", marginBottom: 20 }}>NEWSLETTER</div>
            <p style={{ fontFamily: "var(--font-serif)", fontSize: 13, color: "var(--silver)", marginBottom: 16, lineHeight: 1.6 }}>New drops, exclusive offers — for wolves only.</p>
        ...
        <div style={{ borderTop: "1px solid var(--smoke)", paddingTop: 28, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 20 }}>
            {["Privacy Policy", "Terms", "Shipping Policy"].map(l => (
              <span key={l} style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--silver)", cursor: "pointer", letterSpacing: 1 }}>{l}</span>
            ))}
          </div>
